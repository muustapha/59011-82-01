...
class DAOGenerator
{
	...
	public static function generateDAO(string $className): string
	{
		...
		$code .= "\t\t\$requ = \"INSERT INTO table_\" . \$class . \"(\";\n";
		$code .= "\t\t\$values = \"\";\n";
		$code .= "\t\t\$bindValue = [];\n\n";
		...
		$code .= "\t\t\$class = get_class(\$obj);\n";
		$code .= "\t\t\$colonnes = \$class::getAttributes();\n";
		$code .= "\t\t\$requ = \"UPDATE table_\" . \$class . \" SET \";\n\n";
		...
		$code .= "\t\treturn \$db->query(\"DELETE FROM table_\" . \$class . \" WHERE \" . \$colonnes[0] . \" = \" . \$obj->\$methode());\n";
		$code .= "\t}\n\n";

		$code .= "\tpublic static function count(?array \$nomColonnes = null, string \$class, ?array \$conditions = null, string \$orderBy = null, string \$limit = null, bool \$api = false, bool \$debug = false)\n\t{\n";
		$code .= "\t\t\$nomColonnes = (\$nomColonnes == null) ? \$class::getAttributes() : \$nomColonnes;\n";
		$code .= "\t\t\$liste = self::select(\$nomColonnes,  \$class,  \$conditions,  \$orderBy,  \$limit,  \$api,  \$debug);\n";
		$code .= "\t\tif (\$liste != false)\n\t\treturn count(\$liste);\n";
		$code .= "\t\treturn(0);\n";
		$code .= "\t}\n\n";

		$code .= "\tpublic static function select(?array \$nomColonnes=null, string \$class, ?array \$conditions = null, string \$orderBy = null, string \$limit = null, bool \$api = false, bool \$debug = false)\n\t{\n";
		$code .= "\t\t\$db = DbConnect::getDb();\n";
		$code .= "\t\t\$nomColonnes = (\$nomColonnes == null) ? \$class::getAttributes() : \$nomColonnes;\n";
		$code .= "\t\t\$string = json_encode(\$nomColonnes) . \$class . json_encode(\$conditions) . \$orderBy . \$limit . \$api . \$debug;\n";
		$code .= "\t\tif (strpos((string) \$string, \";\")) {\n";
		$code .= "\t\t\treturn false;\n";
		$code .= "\t\t} else if (!empty(\$nomColonnes)  && (\$class != null && \$class != \"\")) {\n\n";

		$code .= "\t\t\t\$cols = self::elementSelect(\$nomColonnes);\n\n";

		$code .= "\t\t\t\$t = \" FROM table_\". \$class;\n\n";

		$code .= "\t\t\tif (\$conditions != null) {\n";
		$code .= "\t\t\t\t\$conditions = self::conditionSelect(\$conditions,\$class);\n";
		$code .= "\t\t\t}\n";
		...
		$code .= "\tprivate static function conditionSelect(\$conditions,\$class)\n\t{\n";
		$code .= "\t\t\$req = \" WHERE \";\n";
